...
class X509CertificateTests: XCTestCase {
    func testDecoding() {
        guard let certData = Data(base64Encoded: certificatePEM) else { return XCTFail("Failed to load PEM data") }
        XCTAssertNoThrow(try X509Certificate(data: certData))
    }

    func x509() throws -> X509Certificate {
        guard let certData = Data(base64Encoded: certificatePEM) else { fatalError("Failed to parse certificate") }
