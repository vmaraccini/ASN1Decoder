...
class X509CertificateTests: XCTestCase {
    func testDecoding() throws {
        let der = try X509Certificate(data: certificateData)
        let pem = try X509Certificate(data: certificatePEMData)
        XCTAssertEqual(der, pem)
    }

    func testDecodingDERExplicit() throws {
        let pemCertificate = try X509Certificate(der: certificateData)
        XCTAssertEqual(pemCertificate, try x509())
    }

    func testDecodingPEMExplicit() throws {
        let pemCertificate = try X509Certificate(pem: certificatePEMData)
        XCTAssertEqual(pemCertificate, try x509())
    }
